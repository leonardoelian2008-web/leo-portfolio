...
const Gallery = () => {
  ...
  const handlePostComment = async (e) => {
    ...
    try {
      const { success, data } = await apiService.postGalleryComment(
        selectedActivity.id,
        commentForm.name,
        commentForm.content,
      );
    ...
    >
      <div className="text-center mb-8 md:mb-16 px-4">
        <h1 className="text-3xl md:text-6xl font-black font-moderniz tracking-tighter mb-4">
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-blue-600 dark:from-blue-300 dark:to-blue-500 filter drop-shadow-lg">
            ACTIVITY
          </span>
      ...
      {!isLoading && (
        <div className="columns-2 md:columns-3 gap-3 space-y-3 md:gap-6 md:space-y-6 px-2 md:px-8">
          {activities.map((activity) => {
            ...
            const isMulti =
              activity.media_urls && activity.media_urls.length > 1;
            ...
            return (
              ...
              >
                <div className="relative w-full">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6  md:flex">
              ...
              <div className="w-full md:w-[60%] bg-black flex items-center justify-center relative h-[45%] md:h-full group">
                {(() => {
                  const files =
                    ...
                    selectedActivity.media_urls.length > 0
                      ? selectedActivity.media_urls
                  ...
                  const isVideoFile =
                    ...
                    (selectedActivity.type === "video" && files.length === 1);
              ...
              <div className="w-full md:w-[40%] flex flex-col h-[55%] md:h-full border-l dark:border-slate-800 border-slate-200">
                ...
                <div className="p-4 md:p-6 border-b dark:border-slate-800 border-slate-200 flex-shrink-0 bg-white dark:bg-slate-900">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      onClick={() => setSelectedActivity(null)}
                      className="hidden md:block text-slate-400 hover:text-red-500 transition-colors"
                    >
                      ✕
                    </button>
                  </div>
                  <h2 className="text-lg md:text-xl font-bold dark:text-white text-slate-900 mb-1">
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 bg-slate-50 dark:bg-slate-900/50">
                  {loadingComments ? (
                    ...
                    comments.map((comment) => (
                      ...
                      >
                        <div
                          className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-white text-xs ${["bg-red-500", "bg-blue-500", "bg-green-500", "bg-purple-500", "bg-orange-500"][comment.id % 5]}`}
                        >
                          {comment.name.charAt(0).toUpperCase()}
                        </div>
                        <div className="flex-1">
                          <div className="bg-white dark:bg-slate-800 p-3 rounded-r-xl rounded-bl-xl shadow-sm">
                            <p className="text-xs font-bold dark:text-white text-slate-800 mb-1">
                              {comment.name}
                            </p>
                ...
                <div className="p-4 border-t dark:border-slate-800 border-slate-200 bg-white dark:bg-slate-900 flex-shrink-0 pb-safe">
                  <div className="flex items-center gap-4 mb-3">
                    ...
                    >
                      <FaHeart
                        className={`text-xl transition-transform group-active:scale-125 ${selectedActivity.likes > 0 ? "text-red-500" : ""}`}
                      />
                  ...
                  >
                    <div className="flex gap-2">
                      <input
                        ...
                        className="w-1/3 text-sm px-3 py-2 bg-slate-100 dark:bg-slate-800 rounded-lg border-none focus:ring-1 focus:ring-blue-400 dark:text-white"
                        required
                      />
                      <input
                        type="text"
                        ...
                        className="w-2/3 text-sm px-3 py-2 bg-slate-100 dark:bg-slate-800 rounded-lg border-none focus:ring-1 focus:ring-blue-400 dark:text-white"
                        required
                      />
                    </div>
                    <button
                      ...
                      className="ml-auto text-xs font-bold text-blue-600 hover:text-blue-500 disabled:opacity-50 uppercase tracking-wider"
                    >
