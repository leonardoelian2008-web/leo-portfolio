...
const AdminGallery = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (currentItem) {
      // Update
      const result = await apiService.updateGalleryItem(
        currentItem.id,
        payload,
      );
      if (result.success !== false) {
        ...
        >
          ...
          <div className="p-6 border-b dark:border-slate-800 border-slate-200 flex justify-between items-center bg-white/50 dark:bg-slate-900/50 backdrop-blur-md">
            <h2 className="text-2xl font-bold dark:text-white text-slate-800 flex items-center gap-3">
              <FaImage className="text-blue-500" /> Gallery Manager
            </h2>
          ...
          <div className="flex-1 overflow-hidden flex flex-row">
            {/* List Sidebar */}
            <div
              className={`w-full ${isEditing ? "md:w-1/2 hidden md:block" : "w-full"} flex flex-col border-r dark:border-slate-800 border-slate-200`}
            >
              <div className="p-4 overflow-y-auto h-full space-y-3">
                {isLoading ? (
                  <div className="text-center py-10 dark:text-slate-500">
                    Loading gallery...
                  </div>
                ) : galleryItems.length === 0 ? (
                  <div className="text-center py-10 dark:text-slate-500 italic">
                  ...
                  galleryItems.map((item) => (
                    <div
                      key={item.id}
                      className="group flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors border dark:border-slate-800 border-slate-100"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold dark:text-white text-slate-800 truncate">
                          {item.title}
                        </h4>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-2 rounded-lg border dark:border-slate-700 border-slate-300 dark:bg-slate-800 bg-white dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 px-4 py-2 rounded-lg border dark:border-slate-700 border-slate-300 dark:bg-slate-800 bg-white dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                        />
                      </div>
                      <p className="text-xs text-slate-500 mt-1">
                        Paste a direct image link from Unsplash or Supabase
                        Storage.
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full px-4 py-2 rounded-lg border dark:border-slate-700 border-slate-300 dark:bg-slate-800 bg-white dark:text-white focus:ring-2 focus:ring-blue-500 outline-none resize-none"
                      />
                    </div>
                    ...
                    <div className="pt-4 flex justify-end gap-3">
                      ...
                      <button
                        type="submit"
                        className="px-6 py-2 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white font-bold rounded-lg shadow-lg flex items-center gap-2"
                      >
                        <FaSave /> Save Item
                      </button>
                    </div>
